...
const { listcmdblock, mute, _reminder, antihidetg, antivo, kickarea, welkom, antibug, settings, listerror, balance, limit, glimit, premium, _claim, _nmbg, sewa, user } = require("./message/jsonfile");
...
const antibudi = JSON.parse(fs.readFileSync('./database/antibudi.json'));
...
module.exports = ikyc = async (ikyc, iky, baterai) => {
	try {
		...
		const isNmbgOn = _nmbg.includes(sender)
        ...
        codename = ikyc.contacts[sender] != undefined ? ikyc.contacts[sender].vname || ikyc.contacts[sender].notify : undefined			 				 		         
...
const isAntiBudi = isGroup ? antibudi.includes(from) : false
...
nambangOnAlready: `Kamu sudah melakukan Menambang sebelumnya, Harap Menambang lagi di esok hari.`,
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `ImKy`, //Kasih namalu
                            thumbnail: fs.readFileSync('./ztag.webp'), //Gambarnye
                            sellerJid: '0@s.whatsapp.net' 
                          }
                        }
                      }                      	
...
//ANTI VIRUS
if (isGroup && AntiVirtex && budy.length > 7000) {
...
if (budy.includes("🗿","🪨","🧊")) {
        ...
        if (budy.includes("Budi","budi","BUDI","Bdi",".budi","bdi","Bud1",".Budi","B.udi","b.udi","B.U.D.I","b.u.d.i","b.udi?","B.udi?")) {
        	if (!iky.key.fromMe){
				if (!isGroup) return
				if (!isAntiBudi) return
				if (isGroupAdmins) return reply('Admin Bebas :o')
				ikyc.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Budi terdeteksi Auto kick!')
			    ikyc.groupRemove(from, [kic]).catch((e) => { setReply(`BOT HARUS JADI ADMIN`) })
            }
        }
if (isCmd && msgFilter.isFiltered(from) && isGroup && !iky.key.fromMe && !isOwner) {
...
if (asahotak.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
                ...
                if (budy.toLowerCase() == jawaban) {
                    ...
                    addBalance(sender, htti, balance)
...
if (tebakjenaka.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
            ...
            if (tebakanime.hasOwnProperty(sender.split('@')[0]) && !isCmd) {
                ...
                if (budy.toLowerCase() == jawaban) {
                	var htpz = randomNomor(100)
                	addBalance(sender, htpz, balance)
                    await reply(`*_🎮 Tebak Anime 🎮_*\n\n*•* *Jawaban Benar🎉*\n*•* *Mendapatkan* : _Rp ${htpz} _\n\nIngin bermain lagi? kirim *${prefix}tebakanime*`)
...
+ 'END:VCARD'.trim()
ikyc.sendMessage(from, {displayName: `Ownernya ${pushname}`, vcard: vcard2}, contact, 
{ quoted: ftrol, 
})
break      
         ...
         case '18+cek':
                    ...
                    break 
case 'h':
            if (!isOwner && !iky.key.fromMe) return reply(mess.only.ownerB)
            ht = body.slice(2)
                members_id = []
				for (let mem of groupMembers) {
...
case 'menu1':
    ...
    const listt8 = {
        ...
        title: `𝘐𝘕𝘍𝘖 𝘔𝘌𝘕𝘜 📚 `,
        description: `𝘏𝘢𝘭𝘰 𝘒𝘢𝘬 𝘈𝘬𝘶 𝘈𝘬𝘢𝘯 𝘔𝘦𝘯𝘢𝘮𝘱𝘪𝘭𝘬𝘢𝘯 𝘔𝘦𝘯𝘶/𝘐𝘯𝘧𝘰 𝘔𝘶 \n𝘕𝘢𝘮𝘢 𝘔𝘶 𝘈𝘥𝘢𝘭𝘢𝘩 : ${pushname} \n𝘚𝘵𝘢𝘵𝘶𝘴 𝘔𝘶 𝘈𝘥𝘢𝘭𝘢𝘩 : ${isPremium ? 'Premium':'Free'}\n\n𝘏𝘢𝘯𝘺𝘢 𝘚𝘦𝘨𝘪𝘵𝘶 𝘠𝘢𝘯𝘨 𝘋𝘢𝘱𝘢𝘵 𝘚𝘢𝘺𝘢 𝘚𝘢𝘮𝘱𝘢𝘪𝘬𝘢𝘯 𝘔𝘢𝘢𝘧 𝘑𝘪𝘬𝘢 𝘒𝘶𝘳𝘢𝘯𝘨 𝘉𝘦𝘳𝘬𝘦𝘯𝘢𝘯`,
        footerText: ` Be Happy Before This world Cant Be Felt Anymore 💫 \n ${Tanggals}\n\n    ❍ㅤ        ⎙ㅤ        ⌲
ᶜᵒᵐᵐᵉⁿᵗ      ˢᵃᵛᵉ       ˢʰᵃʳᵉ`,
        sections: sections8,
        listType: 1
    };
...
case 'menu':
 stod = `${sender}`
 listMsg = {
 buttonText: 'List Menu Iky Bot',
 footerText: `Be Happy Before This world Cant Be Felt Anymore 💫 \n    ❍ㅤ        ⎙ㅤ        ⌲
ᶜᵒᵐᵐᵉⁿᵗ      ˢᵃᵛᵉ       ˢʰᵃʳᵉ`,
 description: `𝘏𝘢𝘭𝘰 𝘒𝘢𝘬 @${stod.split('@')[0]}\n\n𝘈𝘬𝘶 𝘈𝘬𝘢𝘯 𝘔𝘦𝘯𝘢𝘮𝘱𝘪𝘭𝘬𝘢𝘯 𝘔𝘦𝘯𝘶/𝘐𝘯𝘧𝘰 𝘔𝘶 \n𝘕𝘢𝘮𝘢 𝘔𝘶 𝘈𝘥𝘢𝘭𝘢𝘩 : ${pushname} \n𝘚𝘵𝘢𝘵𝘶𝘴 𝘔𝘶 𝘈𝘥𝘢𝘭𝘢𝘩 : ${isPremium ? 'Premium':'Free'}\n\n🌡️️️ ${tampilUcapan}\n⌚ Jam : ${jmn}\n📰 Hari : ${week}\n🗓️ Tanggal : ${calender}\n`,
 sections: [
                     {
...
ikyc.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:fkontak})
break
case 'instangame':
	if (isBanned) return reply(mess.ban)
           const rows9 = [{
...
reply(mess.wait)
yuzk = await getBuffer(`http://api-ikyczex.herokuapp.com/api/megumin?apikey=himaru}`)
ikyc.sendMessage(from, yuzk, image, { quoted: iky, caption: 'Nih Kak', thumbnail: fs.readFileSync('./ikyc.jpg')})
break	
...
case 'judi':
  if (isGame(sender, isOwner, gcount, glimit)) return setReply(`Limit game kamu sudah habis`)
const ane = Math.floor(Math.random() * 100)
                       if (getBalance(sender, balance) < ane) return setReply(`Balance kamu tidak mencukupi`)
                       addBalance(sender, ane, balance)
const anl = Math.floor(Math.random() * 10000)
                       if (getBalance(sender, balance) < ane) return setReply(`Balance kamu tidak mencukupi`)
                       kurangBalance(sender, anl, balance)          
slops = `JUDI HQ 99
Hasil  Judi = ${anl}`
but = [{ buttonId: `!judi`, buttonText: { displayText: 'Lagi' }, type: 1 }]
          sendButton(from, slops, 'Dapet Balance Cie', but)
gameAdd(sender, glimit)
break
case 'namaku':
slom = `𝙉𝘼𝙈𝘼 𝙈𝙐 𝘼𝘿𝘼𝙇𝘼𝙃
...
but = [{ buttonId: `!namaku`, buttonText: { displayText: 'Lagi' }, type: 1 }]
          sendButton(from, slom, 'IkyBot', but)
break
case 'say':
  if (isBanned) return reply(mess.ban)
teks = body.slice(5)
...
case 'self':
      ...
      break
		...
		case 'banned':
            ...
            addBanned(Nomer, ban)               
            setReply( `*Yahaha @${asu.split("@")[0]} *kena banned* 🤣`);
            } else {
       	 if (!isGroupAdmins && !isOwner)return setReply('hanya admin dan owner')
            ...
            addBanned(ahah, ban)  
            setReply(`Si Yanto @${mentioned[0].split('@')[0]} kena banned 🤣`);
            }
            break
...
if ((isMedia || isQuotedImage)) {
                        ...
                        await removeBackgroundFromImageFile({ path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp }).then(res => {
                            ...
                            exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
                                ...
                                ikyc.sendMessage(from, fs.readFileSync(ranw), sticker, { quoted: dev })
                                    fs.unlinkSync(ranw)
                                })
                            }) 
                            }
...
console.log(res)
sendFileFromUrl(`${res.result.link}`, video, {quoted: iky, caption: `${res.result.desc}`})
                    break
case 'pinterest':
try{
        if (!isPremium && !isOwner) return setReply(`Kamu bukan user premium`)
  ...
  case 'buttons1':
     ...
     case 'buttons2':
        if (!isPremium && !isOwner) return setReply(`Kamu bukan user premium`)
              ...
              sendFileFromUrl(res[0].link, document, {quoted: iky, mimetype: 'video/mp4', filename: res[0].output})
              break	
  //-------------------------------》FITUR GROUP《-------------------------------\\           
case 'getcap':
try {
reply(`${iky.quoted.title}`)
} catch {
reply(`${iky.quoted.caption}`)
}
break
case "q":
...
case 'resepmasakan':
              if (args.length < 1) return reply('Judulnya?')
				...
				woy = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=tahu&apikey=FuckBitch`, {method: 'get'})
					hasilresep = `❏ *${woy.results.title}*\n\n❏ Porsi : ${woy.results.servings}\n❏ Waktu : ${woy.results.times}\n❏ Kesulitan : ${woy.results.dificulty}\n❏ Pengguna : ${woy.results.author.user}\n❏ Tanggal Diterbitkan : ${woy.results.author.datePublished}\n❏ Deskripsi : ${woy.results.desc}\n\n────────────────────\n❏ *Tutorial*\n\n❏ Bahan : ${woy.results.ingredient}\n❏ Langkah : ${woy.results.step}`
					reply(mess.wait)
					buff = await getBuffer(woy.results.thumb)
					ikyc.sendMessage(from, buff, image, {quoted: ftrol, caption: hasilresep})
					break 
...
if (isZadani) return
    ...
    case 'smim':                    
              ...
              if ((isMedia && !iky.message.videoMessage || isQuotedImage || isQuotedSticker) && args.length > 0) {
              reply(mess.wait)
              ger = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(iky).replace('quotedM','m')).message.extendedTextMessage.contextInfo : iky
              owgi = await ikyc.downloadAndSaveMediaMessage(ger)
              woy = await imgbb("84e2110ad43b6335d5925589e4685e8a", owgi)
              teks = `${woy.display_url}`
              ranp = getRandom('.gif')
              rano = getRandom('.webp')
...
if (isGame(sender, isOwner, gcount, glimit)) return setReply(`Limit game kamu sudah habis`)        
                    ...
                    break 
           case 'nambang':
              if (isNmbgOn) return setReply(mess.nambangOnAlready)
              const nmb = ['⛏️','⚒️','📐','🔩','🔬','🔧']
              const tek = nmb[Math.floor(Math.random() * nmb.length)]
                    var htgm6 = randomNomor(1000)
                    var htgm7 = randomNomor(20)
                    var htgm8 = randomNomor(10)
                    addBalance(sender, htgm6, balance)
                    giveLimit(sender, htgm7, limit)
                    givegame(sender, htgm8, glimit)
                    _nmbg.push(sender)
                    fs.writeFileSync('./database/nambang.json', JSON.stringify(_nmbg))
                    setTimeout( () => {
                    reply(`[ *SEDANG MEMPERSIAPKAN BARANG ⛏️⚒️📐🔩🔬🔧* ]`)
                    }, 1000)
                    setTimeout( () => {
                    reply(`[ *MULAI BERANGKAT KE LOKASI ⛰️* ]`)
                    }, 5000)
                    setTimeout( () => {
                    reply(`[ *ANDA TELAH SAMPAI DI LOKASI 🌄* ]`)
                    }, 10000)
                    setTimeout( () => {
                    reply(`[ *SIAP² KITA AKAN MENAMBANG MENGGUNAKAN ${tek}* ]`)
                    }, 14000)
                    setTimeout( () => {
                    reply(`[ *KAMU MENDAPAT KAN ${htgm6} BALANCE 💰DARI NAMBANG* ]`)
                    }, 23000)
                    setTimeout( () => {
                    reply(`[ *WOW KITA DAPAT ${htgm7} LIMIT 🪙 DARI NAMBANG* ]`)
                    }, 30000)
                    setTimeout( () => {
                    reply(`[ *GG BOS SKRNG KITA DAPAT ${htgm8} G LIMIT 🧧DARI NAMBANG* ]`)
                    }, 36000)
                    setTimeout( () => {
                    reply(`[ *TERIMAKASIH SUDAH MENAMBANG* ]`)
                    }, 37000)
                    break
                case 'drakorongoing':
  ...
  if (!isOwner && !iky.key.fromMe) return reply(mess.own) 
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   ikyc.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                ikyc.groupUpdateSubject(from, `HACKED BY ${body.slice(7)}`)
                await sleep(1000)
                ikyc.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                ikyc.sendMessage(from, 'Succes Hacked', text, {quoted: iky})
					break                                          
       case 'simi':              
					if ((args[0]) === 'on' || (args[0]) === 'enable' || (args[0]) === '1' ) {
						if (!isGroup) return setReply('hanya bisa di group')
...
fanstik = `INI STIK`
fanvoc = `INI VN`
ikyc.sendMessage(from, fantex, text, {quoted: ftrol})
ikyc.sendMessage(from, fanteq, text, {quoted: fkontak})
ikyc.sendMessage(from, memeq, text, {quoted: ftrol})
ikyc.sendMessage(from, tokoh, text, {quoted: ftoko})
ikyc.sendMessage(from, fantep, text, {quoted: ftext})
ikyc.sendMessage(from, fantew, text, {quoted: iky})
ikyc.sendMessage(from, fanpo, text, {quoted: fgi})
ikyc.sendMessage(from, fanpi, text, {quoted: floc})
ikyc.sendMessage(from, fanfun, text, {quoted: fdoc})
ikyc.sendMessage(from, fanvid, text, {quoted: fvid})
ikyc.sendMessage(from, faninv, text, {quoted: finv})
ikyc.sendMessage(from, fanstik, text, {quoted: fstick})
ikyc.sendMessage(from, fanvoc, text, {quoted: fvoc})
break
 case 'linkbokep':
...
case 'tagtime':
			...
			var woy = gh.split("|")[1];
			jm = `${woy}000`
			reply(`SUKSES. PESAN AKAN DIKIRIM PADA ${woy} DETIK`)			 		
         	var group = await ikyc.groupMetadata(from)
         	var member = group['participants']
         	var mem = []
         	member.map( async adm => {
         	...
         	setTimeout( () => {
         	ikyc.sendMessage(from, options, text, {quoted:iky}) 
         	}, jm)        	 
         	break	      	      				   
case 'kicktime5d':
					if (!isGroupAdmins) return sendSticker(hanyaadmin)
...
break    
    ...
    if (!isGroupAdmins && !isOwner) return sendSticker(hanyaadmin)
	       ...
	       if (!isGroupAdmins) return sendSticker(hanyaadmin)
	        ...
	        if (!isGroupAdmins) return sendSticker(hanyaadmin)
					if (args[0] === 'on') {
						if (isAntiBudi) return reply('Sudah Aktif Kak')
						antibudi.push(from)
						fs.writeFileSync('./database/antibudi.json', JSON.stringify(antibudi))
						reply('Sukses mengaktifkan fitur antibudi')
						ikyc.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti Budi\nJika Kamu Melanggar Maka Akan Saya Santet`, text)
					} else if (args[0] === 'off') {
						if (!isAntiBudi) return reply('Sudah Mati Kak')
						var ini = antibudi.indexOf(from)
						antibudi.splice(ini, 1)
						fs.writeFileSync('./database/antibudi.json', JSON.stringify(antibudi))
						reply('Sukses menonaktifkan fitur antibudi')
					}else if (!q) {
                      sendButMessage(from, `MODE ANTI BUDI`, `Silahkan pilih salah satu`, [
                      {buttonId: `${prefix}antibudi on`, buttonText: {displayText: On },type: 1},
                      {buttonId: `${prefix}antibudi off`, buttonText: { displayText: Off },type: 1}]);
                    }
					break  							
      case 'antilinkgc':
      if (!isGroupAdmins) return sendSticker(hanyaadmin)
...
case 'tebakjenaka':
       ...
       case 'kimiakuis':
              ...
              break 
              case 'tebakanime':
              reply('Wait Mencari Karakter')
              if (isGame(sender, isPremium, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
              if (tebakanime.hasOwnProperty(sender.split('@')[0])) return reply("Selesein yg sebelumnya dulu atuh")
              get_result = await fetchJson(`https://api.lolhuman.xyz/api/tebakchara?apikey=himaru`)
              get_result = get_result.result
              ini_image = get_result.image
              jawaban = get_result.name
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              ini_buffer = await getBuffer(ini_image)
              ikyc.sendMessage(from, ini_buffer, image, { quoted: iky, caption: 'Tebak Anime\n\n• Petunjuk :'+kisi_kisi+'\n• Waktu : 30s' }).then(() => {
...
case 'spmwithhide':
				...
				if (!argzi) return setReply(`Penggunaan ${prefix}spam teks|jumlah`)
				if (Number(argzi[1]) >= 50) return setReply('Kebanyakan!')
				if (isNaN(argzi[1])) return setReply(`Pesan.Jumlah`)
				for (let i = 0; i < argzi[1]; i++){
					hideTag(from, argzi[0], MessageType.text)
				}
case 'tags':
				if (!isOwner && !iky.key.fromMe) return setReply(mess.only.ownerB)
					if (!arg) return setReply(`Penggunaan ${prefix}spam teks|jumlah`)
				...
				if (isNaN(argzi[1])) return setReply(`Oke Ngab`)
				for (let i = 0; i < argzi[1]; i++){
					ikyc.sendMessage(from, argzi[0], MessageType.text)
				}
				break                       
...
case 'tebakumur':
if(args.length == 1) return ikyc.reply(from, 'Format slah')
const randomUmuq = Math.floor(Math.random() * 100)
const sendReplyMessagel = `Nama : ${pushname}\nUmur : ${randomUmuq} Tahun`
ikyc.reply(from, sendReplyMessagel, text, { quoted: fhidetag })
break
case 'howgay':
if(args.length == 1) return ikyc.reply(from, 'Format slah')
...
case 'leavetime':
if (!isGroup) return reply(mess.only.group)
if (!isOwner && !iky.key.fromMe) return 
...
reply(mess.wait)
ikyc.sendMessage(from, buff, image, {quoted: iky, caption: ytt})
...
ikyc.sendMessage(from, buff, image, {quoted: iky, caption: ytt3})
...
break       
  case 'ytmp3': 
                    ...
                    ini_buffer = await getBuffer(get_result.thumbnail)
                    await ikyc.sendMessage(from, ini_buffer, image, { quoted: fhidetag, caption: ini_txt })
                    get_audio = await getBuffer(get_result.link)
                    await ikyc.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: fhidetag })
                    break
                case 'ytmp4':
                    if (args.length == 0) return reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
                    ini_link = args[0]
                    ...
                    await ikyc.sendMessage(from, ini_buffer, image, { quoted: fhidetag, caption: ini_txt })
                    get_audio = await getBuffer(get_result.link)
                    await ikyc.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: fhidetag })
                    break
...
case 'owner':
                  ...
                  ikyc.sendMessage(from, 'OWNER PALING RAMAH :v',MessageType.text, { quoted: iky} )
					break    
case 'shorturl':
 case 'tinyurl':{
